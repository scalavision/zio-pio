...
class ProcessBuilder private[pio](private[pio] val jProcessBuilder: JProcessBuilder ) {
  ...
  def environment(): ZIO[Any, Exception, Map[String, String]] = IO.effect{
    val mapJs = jProcessBuilder.environment()
    mapJs.asScala.toMap.toMap
  }.refineToOrDie[Exception]
 
  def directory(): ZIO[Any, Exception, java.io.File] = IO.effect(
...
object ProcessBuilder {
  ...
  def apply(arg: String, args: String*) = IO.effect {
