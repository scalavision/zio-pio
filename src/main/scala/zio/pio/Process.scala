...
class Process private[pio](private[pio] val jProcess: JProcess ) {
  ...
  def waitFor(timeout: Duration): IO[InterruptedException, Boolean] = IO.effect {
    val t  = timeout.fold(Long.MaxValue, _.nanos)
    jProcess.waitFor(t, TimeUnit.NANOSECONDS)
  }.refineToOrDie[InterruptedException]

  def waitFor(): IO[InterruptedException, Int] = IO.effect(
    jProcess.waitFor()
  ).refineToOrDie[InterruptedException]

  def exitValue(): IO[IllegalThreadStateException, Int] = IO.effect(
    jProcess.exitValue()
  ...
  // not throw ...?
  def destroy(): IO[Exception, Unit] = IO.effect(
    jProcess.destroy()
  ).refineToOrDie[Exception]

  def destroyForcibly(): IO[Exception, Process] = IO.effect {
    jProcess.destroyForcibly()
    this
  }.refineToOrDie[Exception]

  def isAlive(): IO[Exception, Boolean] = IO.effect(
    jProcess.isAlive()
  ).refineToOrDie[Exception]
